...
os.system('rsync -a --delete rsync://rpki.ripe.net/repository/ ./RPKI_code/rpki')
db = pymysql.connect(host="localhost", user="ken", passwd="123456", database='rpki', charset='utf8')
cursor = db.cursor()
sql_createtable = "create table certificate_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" (Cert_name varchar(100), Cert_URI varchar(200) primary key, Auth_URI varchar(200), Sub_URI varchar(200), Content longtext);"
cursor.execute(sql_createtable)
sql_createtable = "create table roa_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" (ROA_name varchar(100), ROA_URI varchar(200) primary key, Cert_URI varchar(200), Content longtext, foreign key(Cert_URI) references certificate_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+"(Cert_URI));"
cursor.execute(sql_createtable)
sql_createtable = "create table asn_ip_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" (ASN varchar(10), IP_block varchar(50), version varchar(5), max_length varchar(5), ROA_URI varchar(200), foreign key (ROA_URI) references roa_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+"(ROA_URI));"
cursor.execute(sql_createtable)
file_dir = "RPKI_code/rpki"
...
for i in dir_list:
    if '.cer' in i: # ripe
        ...
        param_certificate = (i, url+i, auth, sub, content)
        ...
        for j in dir_list1: # aca
            if '.cer' in j:
                ...
                param_certificate = (j, url + dir + '/' + j, auth, sub, content)
                param_1.append(param_certificate)
                # cursor.execute(sql_insert)
                # cursor.connection.commit()
                f.close()
                ...
                for k in dir_list2: # DEFAULT
                    if '.cer' in k:
                        ...
                        param_certificate = (k, url + dir1 + '/' + k, auth, sub, content)
                        param_1.append(param_certificate)
                        # cursor.execute(sql_insert)
                        # cursor.connection.commit()
                        f.close()
                        ...
                        for l in dir_list3:
                            if '.roa' in l:
                                ...
                                param_roa = (l, dir4, url + dir1 + '/' + k, str(roaContent[0]))
                                # param_roa_1 = [l, dir4, url + dir1 + '/' + k]
                                # param_4.append(param_roa_1)
                                ...
                                for m in range(len_version):
                                    ...
                                    for n in roaContent[0]['ipAddrBlocks'][m]['addresses']:  # roaContent[0]['ipAddrBlocks'][1]['addresses'] ipv6
                                        ...
                                        param_ai = (str(roaContent[0]['asID']), str(IP_block), IPAddressFamily[bytes(roaContent[0]['ipAddrBlocks'][m]['addressFamily'])], str(n['maxLength']), dir4)
                                        param_3.append(param_ai)
                                        # cursor.execute(sql_insert2)
                                # cursor.connection.commit()
# print(param_4)rsync://rpki.ripe.net/repository/DEFAULT/9c/5a0674-f4fc-44a5-958%
# fo = open('2.txt', 'w')
# fo.write(str(param_4))
# fo.close()
try:
    sql_1 = "insert into certificate_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" values(%s, %s, %s, %s, %s);"
    ...
    cursor.connection.commit()
try:
    sql_2 = "insert into roa_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" values(%s, %s, %s, %s);"
    ...
    cursor.connection.commit()
try:
    sql_3 = "insert into asn_ip_"+str(datetime.date.today().year)+"_"+str('{:02d}'.format(datetime.date.today().month))+"_"+str('{:02d}'.format(datetime.date.today().day))+" values(%s, %s, %s, %s, %s);"
